...
def handle_post(parts):
    if len(parts) != 6:
        return b'Error: POST requiere 5 campos'
    try:
        nombre, ancho, alto, profundidad, cantidad = parts[1:]
        conn = get_connection()
        cur = conn.cursor()
        cur.execute(
            "INSERT INTO productos (nombre, ancho, alto, profundidad, cantidad) VALUES (%s, %s, %s, %s, %s)",
            (nombre, float(ancho), float(alto), float(profundidad), int(cantidad))
        )
        conn.commit()
        cur.close()
        conn.close()
        return b'Producto registrado'
...
def handle_get(parts):
    ...
    try:
        ...
        conn = get_connection()
        cur = conn.cursor()
        cur.execute("SELECT * FROM productos WHERE id = %s", (id_,))
        row = cur.fetchone()
        cur.close()
        conn.close()
        if row:
            return f'{row[0]}|{row[1]}|{row[2]}|{row[3]}|{row[4]}|{row[5]}'.encode()
        else:
